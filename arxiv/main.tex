...
\end{itemize}
The study covers three shift types relevant to network security. In \textbf{session and attack-family shift}, training and test data come from sessions with different attack compositions. In \textbf{attack-family shift}, an attack type is entirely absent from training. \textbf{Synthetic feature perturbation} approximates infrastructure-level changes in observed features. These are different dimensions of the same problem: temporal or session variation, coverage of the attack space, and feature-level change.
...
\textbf{Threshold sensitivity.} The threshold sweep confirms that the retained ranking signal is exploitable but bounded. All four models kept ROC-AUC well above 0.5 under shift, so an operator could recover a large fraction of F1 by lowering the threshold. The catch: the threshold must move far from 0.50, to roughly 0.01--0.05, at which point FPR and the share of flagged traffic rise materially. Random Forest's FPR, for example, increases from 0.4\% at threshold 0.50 to 6.5\% at threshold 0.01. A real deployment in benign-dominated traffic would see a proportionally larger false-alarm volume than the 96.1\%-attack test set suggests. Picking the operating threshold without ground-truth labels from the target distribution is a practical challenge.
\textbf{Hybrid architectures.} The differing failure modes across architectures suggest that ensembles mixing model types may be more robust. Tree-based models offer fast inference and high benchmark performance; linear models degrade gracefully under shift. A system combining both could take the strengths of each while covering their weaknesses.
\section{Limitations}
\textbf{Dataset scope.} Only the Friday subset of CICIDS2017 is used (BENIGN, Bot, DDoS, PortScan). The full dataset includes additional attack categories (Brute Force, Heartbleed, Web Attacks, Infiltration) spread across Monday through Friday, which may produce different shift patterns, and these findings may not generalize to datasets with different attack types or network configurations. Future work should replicate this analysis on UNSW-NB15, CICIDS2018, and real production traffic.
\textbf{Session-shift construction.} In Experiment B, the session-based split partitions by attack type (Bot = morning, DDoS/PortScan = afternoon). This approximates but does not perfectly replicate a pure timestamp-level temporal split, because preprocessing concatenates rows by class, partially destroying the original temporal ordering. It is therefore called \textit{session and attack-family shift} rather than \textit{temporal shift}.
\textbf{Train-test size imbalance.} The session split produces substantially different training and test set sizes (13,736 vs. 298,609). The B2 control confirms that distribution shift, not train size, is the primary cause of degradation. The imbalance also shifts class proportions (training is 85.7\% benign; test is 3.9\% benign), an additional variable in the design.
\textbf{Model configurations.} Four architectures are evaluated with fixed, untuned hyperparameters; other configurations might have different robustness profiles. However, the complete collapse of RF and XGB under session shift suggests the main finding is robust to moderate hyperparameter variation. Recurrent, attention-based, and graph-neural architectures are not evaluated; they may behave differently; the ML focus here is deliberate (Section 5.3).
...
\textbf{Binary classification focus.} While multi-class results are reported, the primary analysis uses binary classification (BENIGN vs. ATTACK). Distribution shift may affect multi-class performance differently, especially class-specific detection.
\section{Conclusion}
