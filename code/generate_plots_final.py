"""Regenerate all plots with the updated results (including B2/B3 control experiments)"""
...
# Temporal masks (same as experiments)
...
# ============================================================
# FIGURE 1: Main 4-scenario comparison (A, B, B2, D-control)
# ============================================================
...
# ============================================================
# FIGURE 3: THE KEY FIGURE — Distribution shift vs train-size isolation
# ============================================================
...
ax.set_title('Isolating the Effect of Distribution Shift\n(Same 13K Training Samples: B vs B2)', fontsize=13, fontweight='bold')
...
for ho in attack_classes:
    deg_pairs.append((f'C: {ho}\nLOO', all_results.get('C_attack_family_shift', {}).get(ho, {})))
...
sns.heatmap(cm_b, annot=True, fmt='d', cmap='Reds', ax=axes[1], cbar=False,
            xticklabels=['Benign', 'Attack'], yticklabels=['Benign', 'Attack'], annot_kws={'size': 14})
axes[1].set_title('B: Temporal Shift', fontsize=12, fontweight='bold')
axes[1].set_ylabel('True')
axes[1].set_xlabel('Predicted')

# B2: Matched-size control
np.random.seed(123)
train_idx_b2 = np.random.choice(np.arange(len(X)), size=morning_mask.sum(), replace=False)
sc3 = StandardScaler()
rf_b2 = RandomForestClassifier(n_estimators=80, max_depth=15, random_state=42, n_jobs=-1)
rf_b2.fit(np.nan_to_num(sc3.fit_transform(X[train_idx_b2])), y_bin[train_idx_b2])
cm_b2 = confusion_matrix(y_te_b, rf_b2.predict(np.nan_to_num(sc3.transform(X_te_b))))
...
plt.suptitle('Random Forest Confusion Matrices: Distribution Shift vs Train Size', fontsize=14, fontweight='bold', y=1.02)
plt.tight_layout()
...
# ============================================================
# FIGURE 10: ROC-AUC comparison (binary)
# ============================================================
...
ax.set_title('ROC-AUC Across Scenarios', fontsize=13, fontweight='bold')
ax.set_xticks(x + width * 1.5)
ax.set_xticklabels(exp_labels, fontsize=9)
ax.legend(fontsize=9)
ax.set_ylim(0, 1.1)
